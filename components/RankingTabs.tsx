...
export default function RankingTabs({ records, activeType }: Props) {
  ...
  return (
    <div>
        ...
        {RECORD_TYPES.map((t) => {
          ...
          return (
            <button
              ...
              className={`shrink-0 inline-flex items-center gap-1.5 rounded border px-3 py-1.5 text-[11px] font-bold tracking-[0.18em] transition ${
                active
                  ? "border-arcade-accent bg-arcade-accent/10 text-arcade-accent shadow-[0_0_10px_rgba(255,210,63,0.25)]"
      ...
      {/* SCOREBOARD */}
      <div className="arcade-scanlines relative overflow-hidden rounded border-2 border-arcade-border bg-arcade-panel">
        <div className="border-b border-arcade-border bg-arcade-bg/40 px-3 py-2">
          <div className="flex items-center justify-between text-[10px] tracking-[0.28em] text-zinc-500">
...
function ChampionCard({ champion, unit }: { champion: RecordRow; unit: string }) {
  return (
    <div className="relative border-b border-arcade-accent/30 px-4 py-5 text-center">
      ...
      <div className="text-[10px] tracking-[0.36em] text-arcade-accent">
        ★ ROUND CHAMPION ★
      </div>
      ...
      <div className="mt-3 flex items-center justify-center gap-2 text-[10px] tracking-[0.2em] text-zinc-500">
        <span>{fmtDate(champion.created_at)}</span>
        {isFresh(champion.created_at) && (
          <span className="arcade-blink rounded border border-arcade-danger px-1.5 py-0.5 text-[9px] font-bold tracking-[0.18em] text-arcade-danger">
            NEW!
          </span>
        )}
      </div>
...
function VacantSeat() {
  return (
    <div className="px-4 py-10 text-center">
      <div className="text-[10px] tracking-[0.36em] text-zinc-500">VACANT_SEAT</div>
      <div className="my-3 mx-auto h-px w-16 bg-arcade-border" />
      ...
      <div className="mt-5 text-[11px] tracking-[0.24em] text-arcade-neon">
        <span>{">"}</span>
...
function ChallengerList({
  ...
  return (
    <ol>
      <li className="flex items-center justify-between border-b border-arcade-border/70 bg-arcade-bg/30 px-3 py-1.5 text-[9px] tracking-[0.24em] text-zinc-500">
        <span>RANK · NAME</span>
        <span>SCORE</span>
      </li>

      {challengers.length === 0 && (
        <li className="px-3 py-4 text-center text-[11px] tracking-[0.2em] text-zinc-500">
          <span>NEXT SLOT </span>
          ...
          >
            <div className="flex min-w-0 items-center gap-3">
              ...
              {isFresh(r.created_at) && (
                <span className="arcade-blink shrink-0 rounded border border-arcade-danger px-1 py-px text-[8px] font-bold tracking-[0.18em] text-arcade-danger">
                  NEW
                </span>
              )}
            </div>
